...
$link = mysqli_connect('localhost', 'root', getenv('DB_PASSWORD') ?: '', 'group_17');
    ...
    if ($result = mysqli_query($link, "SELECT * FROM chinese_album ORDER BY no")) {
        ...
        for ($j = 1; $j <= 16; $j++) {
            ...
            $data .= '<div class="col mb-5">
                        <div class="card h-100">
                            <!-- Product image-->
                            <a href="product.php?name='.$row['name']. '&description='.$row['description'].'&price='.$row['price'].'&image=chinese_album_each_page/photo/'.$row['img'].'&music='.$row['music'].'&id='.$row['no'].'&language=chinese">
        ...
        <div class="container px-4 px-lg-5 h-100" style="max-height: 300px; overflow: hidden;">
            <div class="row gx-4 gx-lg-5 h-50 align-items-center justify-content-center text-center">
                <div class="col-lg-9_2 align-self-end">
                    <h1 class="text-white font-weight-bold">中文</h1>
                </div>
            </div>
        </div>
    </header>
