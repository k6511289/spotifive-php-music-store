...
$link = mysqli_connect('localhost', 'root', getenv('DB_PASSWORD') ?: '', 'group_17');
    ...
    if ($result = mysqli_query($link, "SELECT * FROM english_album ORDER BY no")) {
        ...
        for ($j = 1; $j <= 16; $j++) {
            ...
            $data .= '<div class="col mb-5">
                        <div class="card h-100">
                            <!-- Product image-->
                            <a href="product.php?name='.$row['name']. '&description='.$row['description'].'&price='.$row['price'].'&image=english_album_each_page/photo/'.$row['img'].'&music='.$row['music'].'&id='.$row['no'].'&language=english">
...
<body id="page-top">
    ...
    <header class="masthead" style="max-height: 300px; overflow: hidden; min-height: 150px;background: url(assets/img/JB.jpg) 0% 0% / cover, rgba(92, 77, 66, 0.8);">
        <div class="container px-4 px-lg-5 h-100" style="max-height: 300px; overflow: hidden;">
            <div class="row gx-4 gx-lg-5 h-50 align-items-center justify-content-center text-center">
                <div class="col-lg-9_2 align-self-end">
                    <h1 class="text-white font-weight-bold">英文</h1>
                </div>
            </div>
        </div>
    </header>
