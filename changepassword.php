...
if (isset($_SESSION['username'])) {
    ...
    if ($un && isset($_POST['password'])) {
        $pad = $_POST['password'];
        $result = mysqli_query($link, "SELECT * FROM logintest WHERE name = '$un'");
        ...
        if ($result) {
            ...
            if ($rows == 1) {
                ...
                $sql = "UPDATE logintest SET password='" . $_POST['password'] . "'";
                ...
                echo '<script>alert("找不到符合的使用者！");</script>';
            }
        } else {
            echo '<script>alert("SQL執行錯誤！");</script>';
        }
    } else {
        echo '<script>alert("請輸入使用者名稱和密碼！");</script>';
    }
}
            ...
            <div class="container px-4 px-lg-5 h-100" style="display: flex;justify-content: center;align-items: center;">
                <div class="row gx-4 gx-lg-5 h-100 align-items-center justify-content-center text-center">
                    <div class="col-lg-9_2 align-self-end">
                        <h3 class="text-white font-weight-bold">換密碼</h3>
                        <hr class="divider" />
                    </div>
                    <section>
                        <div class="log-container">
                            <form action="" method="post" style="padding-top: 50px;"  id="form1" name="form1">  <!--login.php-->
